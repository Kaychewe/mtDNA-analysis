...
- Smoketest: `stage04_stage04image_smoketest.wdl/json` (checks GOTC tools and GATK image).
  ...
  - Feb 11 update (Stage04 image fix):
    - Built/pushed `kchewe/mtdna-stage04:0.1.3` (bwa, picard wrapper with `--java-options`, samtools, R, Java 11, GATK 4.2.6.0, `python` shim).
    - Added `gotc_docker_override` wiring for `MongoAlignToMtRegShiftedAndMetrics`.
    - Updated `stage04_stage04image_smoketest.json` to use `kchewe/mtdna-stage04:0.1.3` for both GOTC + GATK.
...
- Stage04 GOTC image (BWA/Picard/samtools/R): `kchewe/mtdna-stage04:0.1.3` (Java 11, Picard `--java-options` wrapper, GATK included, `python` shim).
