...
if __name__ == '__main__':
    ...
    sub_id_list = None
    if args.sub_ids:
        with open(args.sub_ids, 'r') as sub_ids_file:
            sub_id_list = [x for x in sub_ids_file.read().splitlines() if x]

    if args.subfolder is None:
        if sub_id_list:
            # Limit output to current batch IDs only.
            subfolders_to_test = sub_id_list
        else:
